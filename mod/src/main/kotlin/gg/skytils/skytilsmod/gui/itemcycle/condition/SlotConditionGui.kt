...
class SlotConditionGui(val cycle: ItemCycle.Cycle, val condition: ItemCycle.Cycle.Condition.SlotCondition? = null) :
    WindowScreen(
        ElementaVersion.V5, newGuiScale = 2
    ) {
    init {
        UIText("Slot Condition for ${cycle.name} (${cycle.uuid})").childOf(window).constrain {
            x = CenterConstraint()
        ...
        val cond = condition ?: ItemCycle.Cycle.Condition.SlotCondition(
            -1000,
            false
        )
        ...
        val slotId by UITextInput("Enter Slot ID").childOf(container).constrain {
            x = SiblingConstraint(10f)
            y = 0.pixels
            width = 30.percent
            height = 10.percent
        }.apply {
            setText(condition?.slotId?.toString() ?: "")
            onLeftClick {
                grabWindowFocus()
            }
